...
func DefaultCatalog() Catalog {
	...
	return NewCatalog(
		...
		card("condition", "Condição", "Controle", "Roteia dados para até oito saídas nomeadas conforme regras.", []Port{in("input", "Entrada", "any", true)}, []Port{out("true", "Verdadeiro", "any"), out("false", "Falso", "any"), out("match_1", "Regra 1", "any"), out("match_2", "Regra 2", "any"), out("match_3", "Regra 3", "any"), out("match_4", "Regra 4", "any"), out("match_5", "Regra 5", "any"), out("match_6", "Regra 6", "any"), out("match_7", "Regra 7", "any"), out("match_8", "Regra 8", "any"), out("default", "Padrão", "any")}),
		...
		card("subpipeline", "Subpipeline", "Controle", "Organiza cards em um container redimensionável com fronteiras de entrada e saída.", nil, nil),
		card("workflow", "Workflow", "Controle", "Executa uma versão publicada de subpipeline por sua interface declarada.", []Port{in("input", "Entrada", "any", false)}, []Port{out("output", "Saída", "any")}),
		card("model", "Modelo IA", "IA", "Executa um modelo por um adaptador de provider sem alterar o contrato da tarefa.", []Port{in("prompt", "Tarefa de review", "review_task", true)}, []Port{out("response", "Resposta", "model_response")}),
		card("validate", "Validar", "Validação", "Valida JSON pelo contrato configurado e aplica regras adicionais de contexto observado.", []Port{in("response", "Resposta", "model_response", true), in("files", "Arquivos buscados", "files", false)}, []Port{out("valid", "Resposta válida", "validated_response"), out("invalid", "Resposta inválida", "error")}),
		card("candidate_validator", "Validar candidatos", "Validação", "Submete cada CandidateFinding a uma decisão independente e libera somente os confirmados.", []Port{in("candidates", "Candidatos", "validated_response", true), in("files", "Contexto observado", "files", false)}, []Port{out("confirmed", "Findings confirmados", "validated_response"), out("decisions", "Decisões internas", "candidate_findings")}),
		...
		card("publish", "Publicar no Gitea", "Saída", "Publica uma revisão formatada por uma integração Gitea ativa, com idempotência durável.", []Port{in("formatted_review", "Review formatada", "formatted_review", true), in("pull_request", "Destino do PR", "pull_request", true)}, []Port{out("receipt", "Comprovante", "publication")}),
