...
func NewCatalog(cards ...CardType) Catalog {
	...
	for _, card := range cards { items[card.Key] = card }
...
func (c Catalog) All() []CardType {
	...
	for _, card := range c.cards { items = append(items, card) }
	return items
}

func DefaultCatalog() Catalog {
	in := func(key, label, contract string, required bool) Port { return Port{Key:key, Label:label, Contract:contract, Required:required} }
