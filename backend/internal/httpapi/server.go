...
func New(catalog workflow.Catalog, workflows *store.SQLite) *gin.Engine {
	...
	api.POST("/workflows", func(c *gin.Context) {
		...
		if err := c.ShouldBindJSON(&definition); err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":err.Error()}); return }
		if err := workflow.Validate(definition, catalog); err != nil { c.JSON(http.StatusUnprocessableEntity, gin.H{"error":err.Error()}); return }
		id, err := workflows.Save(c.Request.Context(), definition)
		if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error":"could not save workflow"}); return }
	...
	api.GET("/workflow-versions/:id", func(c *gin.Context) {
		...
		if err == sql.ErrNoRows { c.JSON(http.StatusNotFound, gin.H{"error":"workflow version not found"}); return }
		if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"error":"could not load workflow"}); return }
