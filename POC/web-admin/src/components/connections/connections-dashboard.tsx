...
export function ConnectionsDashboard({
  ...
  useEffect(() => {
    if (
      selectedID &&
      !data.connections.some((connection) => connection.id === selectedID)
    )
      setSelectedID(null);
  ...
  const modelCount = useMemo(() => {
    ...
    data.models.forEach((model) =>
      count.set(model.connection_id, (count.get(model.connection_id) || 0) + 1),
    );
    return count;
  }, [data.models]);
  const filtered = data.connections.filter((connection) => {
    const provider = providers.get(connection.provider_id);
    return `${connection.name} ${provider?.display_name} ${connection.base_url}`
      .toLowerCase()
  ...
  if (selected) {
    return (
      <ConnectionDetail
        connection={selected}
        provider={providers.get(selected.provider_id)}
        models={data.models.filter(
          (model) => model.connection_id === selected.id,
        )}
        profiles={data.profiles}
        request={request}
        onBack={() => setSelectedID(null)}
        onRefresh={onRefresh}
        onAddModel={() => onAddModel(selected)}
      />
    );
  }

  const defaults = data.connections.filter(
    (connection) => connection.is_default === 1,
  ).length;
  return (
    <>
      <section className="summary-strip">
        <Summary
          ...
          detail={`${data.connections.filter((item) => item.is_enabled === 1).length} ativas`}
        />
        <Summary
          icon={<Cpu />}
          label="Modelos"
      ...
      <section className="connections-section">
        <div className="section-toolbar">
          <div>
            <h2>Suas conexões</h2>
            <p>Selecione uma conexão para ver modelos e configurações.</p>
          </div>
        ...
        {!data.connections.length ? (
          <div className="empty-state">
            ...
            <h3>Crie sua primeira rota de IA</h3>
            <p>
              Conecte Ollama ou OpenRouter e escolha o modelo que fará suas
              revisões.
            </p>
          ...
          <div className="empty-state compact">
            ...
            <p>Tente buscar por outro nome ou provider.</p>
          </div>
        ) : (
            ...
            {filtered.map((connection) => {
              ...
              return (
                <ConnectionCard
                  ...
                  models={modelCount.get(connection.id) || 0}
                  defaultModel={defaultModel}
                  onClick={() => setSelectedID(connection.id)}
                />
              );
            })}
            <button className="new-connection-card" onClick={onNew}>
              <span>
                <Plus size={22} />
              </span>
              <strong>Adicionar conexão</strong>
              <small>Configure uma nova rota</small>
            </button>
          </div>
        )}
      </section>
  ...
  return (
    <button className="connection-card" onClick={onClick}>
      <div className="card-top">
        ...
        >
          <i />
          {connection.is_enabled ? "Ativa" : "Inativa"}
        </span>
      </div>
