...
func (r *Repository) CreatePipelineDraft(ctx context.Context, input PipelineCreateInput) (PipelineInfo, error) {
	...
	version, err := createVersion(ctx, tx, id, 1, input.ProfileID, input.Stages, input.Transitions, input.Triggers)
	if err != nil {
		return PipelineInfo{}, err
	}
...
func (r *Repository) UpdatePipelineDraft(ctx context.Context, pipelineID, versionID int64, input PipelineDraftInput) (PipelineInfo, error) {
	...
	if _, err = tx.ExecContext(ctx, `UPDATE pipeline_versions SET target_profile_id=? WHERE id=?`, input.ProfileID, versionID); err != nil {
		return PipelineInfo{}, err
	}
	if err = tx.Commit(); err != nil {
		return PipelineInfo{}, err
...
func (r *Repository) ClonePublishedPipeline(ctx context.Context, pipelineID, sourceVersionID int64) (PipelineInfo, error) {
	...
	for i, s := range source.Stages {
		inputs[i] = PipelineStageInput{StageTypeKey: s.StageTypeKey, Key: s.Key, Name: s.Name, Prompt: s.PromptTemplate, ModelID: s.ModelID, MaxTokens: s.MaxOutputTokens, RetryLimit: s.RetryLimit, Timeout: s.TimeoutSeconds, UseLLM: s.UseLLM, Required: s.Required, Config: s.Config}
	}
	transitions := transitionInputs(source)
	version, err := createVersion(ctx, tx, pipelineID, next, source.ProfileID, inputs, &transitions, source.Triggers)
	if err != nil {
		return PipelineInfo{}, err
	}
	if err = tx.Commit(); err != nil {
...
func (r *Repository) AdminPipeline(ctx context.Context, pipelineID int64, versionID *int64) (PipelineInfo, error) {
	...
	q := `SELECT id,version,status,target_profile_id,created_at,COALESCE(published_at,'') FROM pipeline_versions WHERE pipeline_definition_id=?`
	args := []any{pipelineID}
	if versionID != nil {
		q += ` AND id=?`
	...
	for rows.Next() {
		...
		if err = rows.Scan(&v.ID, &v.Version, &v.Status, &targetProfile, &v.CreatedAt, &v.PublishedAt); err != nil {
			return out, err
		}
...
func createVersion(ctx context.Context, tx *sql.Tx, pipelineID int64, number int, profileID *int64, stages []PipelineStageInput, transitions *[]PipelineTransitionInput, triggers []PipelineTrigger) (int64, error) {
	result, err := tx.ExecContext(ctx, `INSERT INTO pipeline_versions(pipeline_definition_id,version,status,target_profile_id) VALUES(?,?,'draft',?)`, pipelineID, number, profileID)
	if err != nil {
		return 0, err
	}
	id, _ := result.LastInsertId()
...
func replaceStages(ctx context.Context, tx *sql.Tx, versionID int64, stages []PipelineStageInput, transitions *[]PipelineTransitionInput) error {
	...
	for _, edge := range *transitions {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO pipeline_transitions(pipeline_version_id,from_stage_id,to_stage_id,transition_type,condition_key,priority) VALUES(?,?,?,?,?,?)`, versionID, from, to, edge.Type, edge.ConditionKey, edge.Priority); err != nil {
			return err
		}
	}
	return nil
...
func transitionInputs(definition PipelineDefinition) []PipelineTransitionInput {
	...
	for _, edge := range definition.Transitions {
		item := PipelineTransitionInput{FromStageKey: keys[edge.FromStageID], Type: edge.Type, ConditionKey: edge.ConditionKey, Priority: edge.Priority}
...
func replaceTriggers(ctx context.Context, tx *sql.Tx, versionID int64, triggers []PipelineTrigger) error {
	...
	if len(triggers) == 0 {
		triggers = []PipelineTrigger{{"webhook", true}, {"api", true}, {"manual", true}}
	}
	seen := map[string]bool{}
	for _, trigger := range triggers {
		...
		if _, err := tx.ExecContext(ctx, `INSERT INTO pipeline_version_triggers(pipeline_version_id,trigger_source,is_enabled) VALUES(?,?,?)`, versionID, trigger.Source, boolDB(trigger.Enabled)); err != nil {
			return err
		}
	}
	return nil
