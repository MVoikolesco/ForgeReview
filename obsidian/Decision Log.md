...
## 2026-07-24: Versioned JSON Schema response contracts

The `validate` card owns the response contract as an optional embedded Draft
2020-12 JSON Schema. Official presets are immutable and read-only, while
customization copies the schema into the workflow version. Studio validation is
early feedback only; backend compilation at save/publish and runtime validation
remain authoritative. External references are forbidden and schema size/depth
are bounded. Absence of the setting preserves historical `Finding[]` behavior.
This foundation precedes `CandidateFinding` and independent validation. See
[[Architecture]] and [[Feature Map]].

## 2026-07-24: Candidate proposals cannot publish directly

Review models now propose `CandidateFinding` records without assigning their
own status. A separate `candidate_validator` call evaluates exactly one
candidate and returns only confirm, reject, or needs-context with a reason.
System-owned observability checks may assign `NOT_OBSERVABLE`; no malformed or
failed validation path confirms by fallback. Only confirmed candidates become
public `Finding` records. Internal decisions remain in retained execution
evidence, and untouched official seeds receive the new graph through an
append-only version upgrade. See [[Architecture]], [[Feature Map]], and
`docs/findings-verificaveis.md`.

## 2026-07-24: Finding identity excludes generated prose

Candidate fingerprints are generated by the system from repository, base
commit, path, symbol, check ID, issue type, and affected entity. Claim wording,
publication comment, severity, and line number are excluded because they are
unstable model output or presentation coordinates. Duplicate candidates are
rejected before independent model validation, and confirmed findings are
deduplicated again at consolidation. Legacy findings without a fingerprint keep
their prior textual compatibility key. See [[Architecture]], [[Feature Map]],
and `docs/findings-verificaveis.md`.

## 2026-07-24: Reviewer checklists are closed version snapshots

Checklist catalog entries are immutable and copied into each workflow version.
The reviewer may emit candidates only for listed check IDs, and the independent
validator deterministically marks outside IDs `NOT_APPLICABLE` without provider
cost. This makes execution reproducible even after later catalog versions are
introduced. Workflows without a checklist retain compatibility. See
[[Architecture]], [[Feature Map]], and `docs/review-checklists.md`.
