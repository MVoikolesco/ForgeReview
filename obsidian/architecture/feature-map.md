...
- Ollama integrations: the connection wizard persists an explicit authentication classification; local Ollama needs no key, while Ollama Cloud stores and uses a connection-scoped encrypted key through the same Ollama client contract.
- Pipeline LLM editável: quatro Markdown obrigatórios definem instruções técnicas, segurança/performance, divergência de importação e formato final; filtros de arquivos permanecem no YAML.
...
Ollama Cloud catalog and review execution depend on network access to the configured Ollama-compatible endpoint and a persisted encrypted API key.
